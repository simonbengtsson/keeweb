...
import { getDatabase, getInstance, setupApp } from './flownservices.ts';
...
app.get('/api/read', async () => {
    const instance = getInstance();
    const kv = await getDatabase();
    const res = await kv.get<Uint8Array>(['files', instance.id]);
    const array = res.value || new Uint8Array();
    return new Response(array.buffer);
});

app.post('/api/save', async (ctx) => {
    const body = await ctx.req.parseBody();
    console.log('sdf', body instanceof Uint8Array);
    // const instance = getInstance();
    // const kv = await getDatabase();
    // await kv.set(['files', instance.id], body);
